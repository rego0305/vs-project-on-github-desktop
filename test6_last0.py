...
class SocketGUI:
    ...
    def authentication(self):
        ...
        while True:
            ...
            self.conn.sendall(str(challenge).encode())  #int → str → bytes ， 1234 → '1234' → b'1234'
            # 发送
    ...
    def connect_ip(self):
        ...
        if self.button1_clicked and self.button2_clicked:
            ...
            self.start_button2.config(state=tk.NORMAL)    #  验证响应启用，因为这是客户机不是服务器    # 输入口令密码，计算响应值并发送之
    ...
    def receive_messages(self):
        ...
        while True:
            try:
                ...
                # self.textfield_message1.insert(tk.END, f"\n[ {self.HISname}_1 ]:        \n{message}\n")
                self.textfield_message1.insert(tk.END, f"\n[  Him  ]:        \n{message}\n")
                # 获取主机名称？在发送消息的时候，就已经把自己主机名和日期给带上了，所以不需要单独发送主机名给对方
