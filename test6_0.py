...
class SocketGUI:
    def __init__(self, root):
        ...
        # self.challenge = Entry(self.top_panel1)
        # self.challenge.pack(side=tk.LEFT, padx=10)  # Authentication
        ...
        # self.start_button2 = tk.Button(self.top_panel1, text="发送响应值，输入密码", command=self.authentication_client(),width=10)
        # self.start_button2 = tk.Button(self.top_panel1, text="发送响应值，输入密码", command=self.authentication_client, width=20, state = tk.DISABLED)
        self.start_button2 = tk.Button(self.top_panel1, text="输入密码，计算并发送响应值", command=self.password_client,width=30, state = tk.DISABLED)
        # ？？？一开始的时候禁用按钮，在发出连接请求后启动。 command 应该是password而不是authentication    command=self.password_client也是一样会报错

        # 这里把self.authentication_client()改成self.authentication_client，文件就不会报错，为什么？？？？
        # command=...是告诉 Tkinter：当用户点击这个按钮时，要去调用哪一个函数。
        # 关键问题：为什么现在写错了？你写了 self.authentication_client() ——后面带了括号。
        # 在 Python 里，有括号表示“立刻执行这个函数”，并把返回值当作 command 传给按钮。
        # 结果：按钮还没显示出来，函数就被立即执行了；此时 client_socket 还没建立，于是 260 行报错。
        # 去掉括号，Tkinter 才会在用户真正点击按钮时才去调用 authentication_client，而不是在创建按钮时就执行。
        self.start_button2.pack(side=tk.LEFT, padx=25)  # 把按钮的位置放在左边
        ...
        # self.button_message2 = Button(container_message2, text="发送消息", width=15 , padx= 5, command=self.send)
        self.button_message2 = Button(container_message2, text="发送消息", width=15, padx=5, command=self.send, state=tk.DISABLED)
    ...
    def start_listening(self):
        if self.button1_clicked and self.button2_clicked:
            ...
            # 点过一次开始监听后，就不能再点了。
            # self.button1_clicked和self.button1_clicked分别是获取ip和主机名
        else:
    ...
    def listen_for_connection(self):  # 开始监听 就是 开放连接
        ...
        # 下面这一行注释掉，在验证通过后再启动receive进程
        # threading.Thread(target=self.receive_messages).start()  # 他妈的，之前在connect_ip()那一块忘记加了这一行代码。

        threading.Thread(target=self.authentication).start()  # 开始身份验证阶段 ，服务器在接收到连接请求后，就开始authentication线程
    ...
    def connect_ip(self):
        ...
        if self.button1_clicked and self.button2_clicked:
            ...
            # self.start_button2.config(state=tk.DISABLED)  # 验证响应禁用，因为这是客户机不是服务器
            ...
            threading.Thread(target=self.authentication_client).start()  # 开始身份验证阶段，客户端在发送连接请求后，就开始authentication_client线程
    ...
    # def name_exchange(self):  # 服务器的12345端口  连接与  客户机的45678端口  怎么防止数据碰撞？
    #     pass
    ...
    def authentication(self):  # 服务器端的认证 authentication_server
        # 产生随机挑战值
        while True:
            # 生成随机数作为挑战
            challenge = random.randint(1, 9999)     # ？？？ None

            # challenge = self.generate_challenge()  # 之前是self.generate_challenge()部分没写
            ...
            if received_response == expect_response:
                ...
                print(received_response)   # ？？？ 测试用的代码 ，把收到的响应和期望的响应都打印出来，比较一下
    ...
    def authentication_client(self):  # 客户端的认证
        while True:
            # 接收服务器发来的挑战
            challenge_str = self.client_socket.recv(1024).decode()   # ？？？这个是recv1
            # if not challenge_str:
            #     break  # 服务器关闭连接，结束循环
            print(f"challenge_recv ： {challenge_str}")
            ...
            # 接收服务器对响应的验证结果
            server_response = self.client_socket.recv(1024).decode()        # ？？？这个是recv2，recv了两次
            ...
            if server_response == "Authentication success. 身份认证成功":  # 回应信息该显示在何处？
                ...
                # self.start_button.config(state=tk.NORMAL)  # 启用信息发送按钮
                self.button_message2.config(state=tk.NORMAL)  # button_message2才是真正的发送信息按钮  ？？？这里做了修改
                threading.Thread(target=self.receive_messages).start()  # ？？？ 这里做了修改
                break  # 输入正确，结束循环
